...
/* Variables for foo share methods */

/* 
   Hash used to track the number of open tables; variable for foo share
   methods
*/
static HASH foo_open_tables;

/* The mutex used to init the hash; variable for foo share methods */
mysql_mutex_t foo_mutex;
...
static uchar* foo_get_key(FOO_SHARE *share, size_t *length,
...
static PSI_mutex_key ex_key_mutex_foo, ex_key_mutex_FOO_SHARE_mutex;

static PSI_mutex_info all_foo_mutexes[]=
{
  { &ex_key_mutex_foo, "foo", PSI_FLAG_GLOBAL},
  { &ex_key_mutex_FOO_SHARE_mutex, "FOO_SHARE::mutex", 0}
};

static void init_foo_psi_keys()
{
  const char* category= "foo";
  int count;

  if (PSI_server == NULL)
    return;

  count= array_elements(all_foo_mutexes);
  PSI_server->register_mutex(category, all_foo_mutexes, count);
}
#endif
...
static int foo_init_func(void *p)
{
  ...
  mysql_mutex_init(ex_key_mutex_foo, &foo_mutex, MY_MUTEX_INIT_FAST);
  (void) my_hash_init(&foo_open_tables,system_charset_info,32,0,0,
                      (my_hash_get_key) foo_get_key,0,0);
...
static int foo_done_func(void *p)
{
  int error= 0;
  DBUG_ENTER("foo_done_func");

  if (foo_open_tables.records)
    error= 1;
  my_hash_free(&foo_open_tables);
  mysql_mutex_destroy(&foo_mutex);
...
static FOO_SHARE *get_share(const char *table_name, TABLE *table)
{
  FOO_SHARE *share;
  uint length;
  char *tmp_name;

  mysql_mutex_lock(&foo_mutex);
  length=(uint) strlen(table_name);

  if (!(share=(FOO_SHARE*) my_hash_search(&foo_open_tables,
                                              (uchar*) table_name,
                                              length)))
  {
    if (!(share=(FOO_SHARE *)
    ...
    if (my_hash_insert(&foo_open_tables, (uchar*) share))
      goto error;
    thr_lock_init(&share->lock);
    mysql_mutex_init(ex_key_mutex_FOO_SHARE_mutex,
...
static int free_share(FOO_SHARE *share)
{
  mysql_mutex_lock(&foo_mutex);
  if (!--share->use_count)
  {
    my_hash_delete(&foo_open_tables, (uchar*) share);
...
*/
static st_system_tablename ha_foo_system_tables[]= {
  {(const char*)NULL, (const char*)NULL}
};
...
/**
  ...
  and updated timestamp field. You can do these for foo by doing:
  @code
...
{
  {"foo_func_foo",  (char *)show_func_foo, SHOW_FUNC},
  {0,0,SHOW_UNDEF}
};
