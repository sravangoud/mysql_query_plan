...
static void select_describe_custom( JOIN *join,
									bool need_tmp_table,
									bool need_order,
									bool distinct,
									const char *message=NullS );

extern TABLE_LIST **make_leaves_list(TABLE_LIST **list, TABLE_LIST *tables);
	      ...
	      SELECT_LEX_UNIT *unit_arg)
{
  ...
  */
  ...
  if (!(select_options & OPTION_SETUP_TABLES_DONE) &&
      ...
      DBUG_RETURN(-1);
#else
  make_leaves_list(&select_lex->leaf_tables, tables_list);
#endif
...
static void select_describe_custom(JOIN *join, bool need_tmp_table, bool need_order,
			    bool distinct,const char *message)
{
  List<Item> field_list;
  List<Item> item_list;
...
//  {
	...
	item_list.push_back(new Item_string(real_table->alias,
      ...
      /* "partitions" column */
      if (true /*join->thd->lex->describe*/ & DESCRIBE_PARTITIONS)
      {
#ifdef WITH_PARTITION_STORAGE_ENGINE
        partition_info *part_info;
        if (!table->derived_select_number &&
	...
	item_list.push_back(item_null);
      ...
      {
        /* in_rows */
        if (/*join->thd->lex->describe*/ true & DESCRIBE_EXTENDED)
          item_list.push_back(item_null);
        /* rows */
        item_list.push_back(item_null);
      }
